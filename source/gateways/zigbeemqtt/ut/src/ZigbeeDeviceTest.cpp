...
public:
    ...
    std::shared_ptr<NiceMock<MqttClientMock>> mqttClientMockPtr = std::make_shared<NiceMock<MqttClientMock>>();
...
TEST_F(ZigbeeDeviceTest, shouldAddCapabilitiesDescribedInConfig)
{
    ...
    configuration::Configuration config{
        ...
        {{device::CapabilityType::Sensor, device::SensorType::Pressure, "testPath", std::nullopt},
         {device::CapabilityType::Sensor, device::SensorType::Humidity, "secondCapability", "Bob"}}};
    ...
    EXPECT_THAT(
        ...
        UnorderedElementsAre(
            Pointee(Property(&ICapability::getSubType, device::SensorType::Pressure)),
...
TEST_F(ZigbeeDeviceTest, shouldUpdateCapabilitiesWhenUpdateReceived)
{
    ...
    EXPECT_THAT(
        ...
        UnorderedElementsAre(
            Pointee(AllOf(
                Property(&ICapability::getSubType, device::SensorType::Pressure),
                Property(&ICapability::getValue, DoubleEq(12.34)))),
            Pointee(AllOf(
                Property(&ICapability::getSubType, device::SensorType::Humidity),
